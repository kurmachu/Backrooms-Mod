...
public class BlockLightDirty extends BlockContainer {

    public BlockLightDirty() {
        super(Material.GLASS);
        setSoundType(SoundType.GLASS);
        setRegistryName("lightondirty");
        setUnlocalizedName("lightondirty");
    ...
    public static void onBreakEvent(BlockEvent.BreakEvent event) {
        if(event.getState().getBlock().equals(BackroomBlocks.blockLightDirty)){
            if(!event.getPlayer().isCreative()){
                ...
                if(random.nextInt(100)<50){
                    ...
                    ItemStack egg = new ItemStack(Blocks.DIRT);
                    EntityItem item2 = new EntityItem(event.getWorld(),event.getPos().getX()+0.5,event.getPos().getY()-0.9,event.getPos().getZ()+0.5,egg);
                    event.getWorld().spawnEntity(item2);
                }
